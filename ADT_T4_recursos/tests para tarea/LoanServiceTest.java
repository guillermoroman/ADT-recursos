...
public class LoanServiceTest {
    ...
    public void testReturnItem() {
        // Primero, realizamos un préstamo
        loanService.loanItemToUser(1L, 1L); // Asegúrate de que esto es posible en tu configuración de prueba

        // Luego, intentamos devolver el ítem
        String returnResult = loanService.returnItem(1L); // Asumiendo que el ítem con ID 1L fue prestado anteriormente
        assertEquals("Item devuelto correctamente", returnResult, "El ítem debería ser devuelto correctamente.");
    }

    @Test
    public void testGetItemsLoanedToUser() {
        assertNotNull(loanService.getItemsLoanedToUser(1L)); // Verificar que la lista no es nula
    }
    ...
    public void testGetAllLoansFromItem() {
        assertNotNull(loanService.getAllLoansFromItem(1L)); // Verificar que la lista no es nula
    }
